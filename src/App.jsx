...
export default function App() {
  ...
  // Estados locales como fallback para el modo offline / sin pareja conectada
  const [listaLocal, setListaLocal] = useState(() => {
    const s = localStorage.getItem('mercadito_lista')
    return s ? JSON.parse(s) : []
  })
  ...
  // Sincronización con LocalStorage (modo local)
  useEffect(() => {
    if (!grupoId) localStorage.setItem('mercadito_lista', JSON.stringify(listaLocal))
  }, [listaLocal, grupoId])
  ...
  // Selección dinámica de datos (Firestore vs Local)
  const lista = grupoId ? listaFirestore : listaLocal
  const historial = grupoId ? historialFirestore : historialLocal
  const precios = grupoId ? preciosFirestore : preciosLocal
  const catalogoCustom = grupoId ? catalogoCustomFirestore : catalogoCustomLocal
  ...
  // Huevo de pascua / Saludo de Sully los viernes
  useEffect(() => {
    const hoy = new Date()
    const esViernes = hoy.getDay() === 5
    const yaVio = localStorage.getItem('mercadito_viernes_' + hoy.toDateString())
  ...
  // Modificado para insertar el producto con la cantidad que viene del catálogo
  const agregarItem = (item) => {
    setLista(prev => {
      const existeIndex = prev.findIndex(i => i.id === item.id)
